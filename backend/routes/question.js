...
router.post("/", async (req, res) => {
  try {
    ...
    db.questions.push(questionToSave);
    writeJSON(db);
...
router.post("/teacher", async (req, res) => {
  try {
    ...
    const db = readJSON();
    const newQuestion = { _id: makeId("q"), ...questionData, teacherId };
...
router.get("/", async (req, res) => {
  try {
    const db = readJSON();
    const questions = safeArray(db.questions);
...
router.get("/:id", async (req, res) => {
  try {
    const db = readJSON();
    const question = safeArray(db.questions).find((x) => String(x._id) === String(req.params.id));
...
router.put("/:id", async (req, res) => {
  try {
    const db = readJSON();
    const index = safeArray(db.questions).findIndex((x) => String(x._id) === String(req.params.id));
    if (index === -1) return res.status(404).json({ message: "Question not found" });
    const updated = { ...db.questions[index], ...req.body };
...
router.delete("/:id", async (req, res) => {
  try {
    const db = readJSON();
    const before = safeArray(db.questions).length;
    db.questions = safeArray(db.questions).filter((x) => String(x._id) !== String(req.params.id));
    const deleted = before !== db.questions.length;
    writeJSON(db);
...
router.get("/topics/:class/:subject", async (req, res) => {
  try {
    ...
    const topics = [
      ...new Set(
        safeArray(db.questions)
          .filter((q) => String(q.class) === String(className) && String(q.subject) === String(subject))
          .map((q) => q.topic)
          .filter(Boolean),
      ),
    ];
...
router.get("/:class/:subject/:topic", async (req, res) => {
  try {
    ...
    const db = readJSON();
    const questions = safeArray(db.questions).filter(
      (q) =>
        String(q.class) === String(className) &&
        String(q.subject) === String(subject) &&
        String(q.topic) === String(topic),
    );
    res.status(200).json(questions);
  } catch (err) {
    res.status(500).json({ error: err.message });
